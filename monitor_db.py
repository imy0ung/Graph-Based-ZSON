...
def get_db_stats(db_path: str):
    ...
    try:
        ...
        stats['region_nodes'] = cursor.fetchone()[0]
        
        # Region nodes by category (name)
        cursor.execute("""
            ...
            ORDER BY count DESC
        """)
        stats['region_categories'] = {row['name']: row['count'] for row in cursor.fetchall()}
    except sqlite3.OperationalError:
        # Table doesn't exist yet (old schema)
        stats['region_nodes'] = 0
        stats['region_categories'] = {}
    ...
    try:
        while True:
            ...
            if stats is None:
                ...
                print(f"  Region Nodes:   {stats.get('region_nodes', 0):>6}")
                ...
                if stats.get('region_categories'):
                    print(f"\n🏘️  REGION CATEGORIES")
                    for name, count in stats['region_categories'].items():
                ...
                if last_stats is not None:
                    ...
                    region_delta = stats.get('region_nodes', 0) - last_stats.get('region_nodes', 0)
                    ...
                    if region_delta != 0:
                        print(f"  Region Nodes:   {region_delta:>+6}")
                    ...
                    if stats.get('region_categories') and last_stats.get('region_categories'):
                        all_categories = set(list(stats['region_categories'].keys()) + list(last_stats['region_categories'].keys()))
                        category_changes = False
                        for category in all_categories:
                            old_count = last_stats['region_categories'].get(category, 0)
                            new_count = stats['region_categories'].get(category, 0)
                            if new_count != old_count:
                                if not category_changes:
